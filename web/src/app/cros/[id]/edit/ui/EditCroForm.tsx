...
//   const [contact, setContact] = useState(cro.contact_email ?? "");
...
//           website,
//           country,
//           contact,               // API maps this → contact_email
//           specialties,           // string OK; API normalizes to array
//         }),
//       });
...
//         <div className="text-xs text-slate-600">Updates reflect on your public page</div>
//       </div>
...
export default function EditCroForm({ cro }: { cro: CroRow }) {
  const router = useRouter();
  const [name, setName] = useState<string>(cro.name ?? "");
  const [website, setWebsite] = useState<string>(cro.website ?? "");
  const [country, setCountry] = useState<string>(cro.country ?? "");
  const [contact, setContact] = useState<string>(cro.contact_email ?? "");
  const [specialties, setSpecialties] = useState<string>((cro.specialties ?? []).join(", "));
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <Field label="Specialties (comma separated)">
        <input
          className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-slate-300 placeholder:text-slate-400 focus:ring-2"
          value={specialties}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setSpecialties(e.target.value)}
          placeholder="Oncology, Endocrinology, Rare Disease"
        />
      </Field>
