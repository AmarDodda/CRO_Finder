...
const BodySchema = z.object({
  ...
  sponsor_email: z.string().trim().email().optional(),
...
export async function POST(req: Request) {
  try {
    ...
    const { cro_id, project_id, sponsor_email, message } = parsed.data;

    // Next 15 cookie adapter for @supabase/ssr
    const jar = await cookies();
    const cookieMethods: CookieMethodsServer = {
      getAll: () => jar.getAll(),
      setAll: (list) => {
    ...
    // Ensure CRO exists (and get its owner)
    const { data: cro, error: croErr } = await supabaseAdmin
      .from("cros")
      .select("id, owner")
      .eq("id", cro_id)
    ...
    const trimmedEmail = sponsor_email ?? "";
    ...
    const { data: inserted, error: insErr } = await supabaseAdmin
      .from("contacts")
      .insert({
        recipient: cro.owner,              // can be null if CRO owner not set
        sender: user?.id ?? null,          // allow anonymous inquiries
        project_id: project_id ?? null,
        message: messageText,
        sent_at: new Date().toISOString(),
        read_at: null,
    ...
    // Best-effort analytics/notification (ignore errors)
