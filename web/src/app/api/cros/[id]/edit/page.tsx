// app/cros/[id]/edit/page.tsx
import "server-only";
import { cookies } from "next/headers";
import Link from "next/link";
import { createServerClient, type CookieMethodsServer } from "@supabase/ssr";

import EditCroForm from "../../../cros/[id]/edit/ui/EditCroForm";
...
export default async function CroEditPage({ params }: { params: { id: string } }) {
  ...
  // Verify ownership & load CRO
  const { data: cro, error } = await sb
    .from("cros")
    .select("id,name,website,country,contact_email,specialties,owner")
    .eq("id", params.id)
    .eq("owner", user.id) // enforce ownership
    .maybeSingle();
        ...
        <div className="mx-auto max-w-3xl px-6 py-14">
          <div className="rounded-2xl border border-rose-200 bg-rose-50/80 p-6 text-rose-900 shadow-sm">
            ...
            <p className="mt-1 text-sm">
              We couldn’t find that CRO profile or you don’t have permission to edit it.
            </p>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur">
            ...
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              <li><span className="h-1.5 w-1.5 rounded-full bg-violet-500 inline-block mr-2" /> Use your official website & contact email.</li>
              <li><span className="h-1.5 w-1.5 rounded-full bg-sky-500 inline-block mr-2" /> Add specialties (e.g., Oncology, Rare Disease, PK/PD).</li>
              <li><span className="h-1.5 w-1.5 rounded-full bg-emerald-500 inline-block mr-2" /> Keep country/location accurate for matching.</li>
            </ul>
            <Link href="/cros/me" className="mt-4 inline-block text-sm text-white rounded-xl bg-slate-900 px-4 py-2">
              Back to dashboard
            </Link>
          </aside>
        </div>
