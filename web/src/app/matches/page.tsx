...
function MatchesInner() {
  ...
  const [minSim, setMinSim] = useState<number>(0.5);
  const [sort, setSort] = useState<"best" | "name" | "country">("best");
  const [expanded, setExpanded] = useState<Record<string, boolean>>({});
    ...
    (async () => {
      ...
      try {
        ...
        const r = await fetch("/api/match-project", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ project_id: projectId, k: 18 }),
        });
        const j = (await r.json()) as ApiResponse & { error?: string };
        if (!r.ok) throw new Error(j.error ?? "Failed to load matches");
        setData(j);
            ...
            >
              ...
              <div className="mt-4">
                ...
                <input
                  ...
                  step={5}
                  value={Math.round(minSim * 100)}
                  onChange={(e: ChangeEvent<HTMLInputElement>) => setMinSim(Number(e.target.value) / 100)}
                  className="w-full"
                />
              </div>
              ...
              <label className="mt-4 block">
                ...
                <select
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                    setSort(e.target.value as "best" | "name" | "country")
                  }
                >
                  <option value="best">Best match</option>
                  <option value="name">Name (A–Z)</option>
...
//   useEffect(() => {
//     (async () => {
//       // Validate client-side first so we don't call the API with a bad id
//       if (!projectId) return;
//       if (!isUuid(projectId)) {
//         setErr("Invalid projectId in URL.");
//         setData(null);
//         return;
//       }
...
//         // Optional: auto-relax threshold to keep at least the best match visible
//         const best = Math.max(
//           0,
//           ...((j.results ?? []).map((x) => x.similarity ?? 0) as number[])
//         );
//         if (best > 0 && best < minSim) {
//           setMinSim(best); // relax slider so you can see the top result
//         }
//       } catch (e: unknown) {
//         const msg = e instanceof Error ? e.message : "Error";
//         setErr(msg);
...
//   // 1) Missing or invalid projectId
//   if (!projectId) {
//     return (
//       <main className="min-h-[100dvh] bg-gradient-to-b from-indigo-50 via-white to-white text-slate-800">
//         <div className="mx-auto max-w-3xl px-6 py-10">
...
//             body="Open this page via Create & Match or include ?projectId=…"
//           />
//         </div>
//       </main>
//     );
...
//       <main className="min-h-[100dvh] bg-gradient-to-b from-indigo-50 via-white to-white text-slate-800">
//         <div className="mx-auto max-w-3xl px-6 py-10">
//           <ErrorCard message="Invalid projectId in URL." />
//         </div>
//       </main>
//     );
//   }

//   // 2) Network / API errors (e.g. "No project matched")
//   if (err) {
//     return (
//       <main className="min-h-[100dvh] bg-gradient-to-b from-indigo-50 via-white to-white text-slate-800">
//         <div className="mx-auto max-w-3xl px-6 py-10">
...
//     <main className="min-h-[100dvh] bg-gradient-to-b from-indigo-50 via-white to-white text-slate-800">
//       {/* Top banner */}
//       {/* ... keep your existing banner markup ... */}

//       {/* Body */}
//       <div className="mx-auto max-w-6xl px-6 py-8">
//         <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
//           {/* Sidebar ... unchanged except handlers are typed */}
...
//               <SkeletonList />
//             ) : deduped.length === 0 ? (
//               // 3) API returned OK, but no matches at all for this project
//               <EmptyCard
//                 title="No matches for this project"
//                 body="We couldn’t find suitable CROs for the provided details yet. Try broadening criteria or creating another project."
//                 cta={{ href: `/projects/new`, label: "Create another project" }}
//               />
//             ) : filtered.length === 0 ? (
//               // 4) There ARE results, but your current filters hide them
//               <EmptyCard
//                 title="No matches after filtering"
//                 body="Try lowering the minimum match, clearing specialties, or adjusting your search terms."
//               />
//             ) : (
//               /* your list rendering exactly as you had it */
//               /* ... */
//               <ul className="space-y-4">
//                 {/* unchanged mapping content */}
//               </ul>
//             )}
//           </section>
//         </div>
